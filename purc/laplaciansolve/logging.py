"""
Logging configuration for the LaplacianSolve package.

Provides the same ``get_logger`` / ``logger`` / ``configure_logging`` surface as
the PUM package so the two share a consistent logging style.
"""
...
def _configure_base_logger() -> logging.Logger:
    ...
    Returns:
        The base ``"purc.laplaciansolve"`` logger.
    ...
    return logging.getLogger("purc.laplaciansolve")
    ...
    *,
    ...
    Args:
        ...
        package_levels: Per-package level overrides, e.g.
            ``{"purc.laplaciansolve.reference": logging.DEBUG}``.
