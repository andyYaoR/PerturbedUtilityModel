    ...
    def _fast_backtrack_batch(self, x, z, w, lam, dx, dz, dw, dlam, ap_max, ad_max, mu, done):
        ...
        if native_available():
            ...
            native_core().ipm_fast_backtrack_f64(
                f(x), f(z), f(w), f(lam), f(dx), f(dz), f(dw), f(dlam),
                f(ap_max), f(ad_max), f(mu), to_numpy(done).astype(np.uint8).reshape(-1),
                float(self.rho), float(self.gamma_min), float(self.ls_chi), int(self.ls_max),
                B, N, k, xf, zf, wf, lamf, do,
            )
            return (
    ...
    def _safe_backtrack_batch(self, x, z, w, lam, dx, dz, dw, dlam, ap_s, ad_s, mu, sigma_s, done):
        ...
        if native_available():
            ...
            native_core().ipm_safe_backtrack_f64(
                f(x), f(z), f(w), f(lam), f(dx), f(dz), f(dw), f(dlam),
                f(ap_s), f(ad_s), f(mu), f(sigma_s), to_numpy(done).astype(np.uint8).reshape(-1),
                float(self.kappa), float(self.gamma_min), float(self.ls_chi), int(self.ls_max),
                B, N, k, xs, zs, ws, lams, do,
            )
            return (
