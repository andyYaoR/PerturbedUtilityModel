...
bounds $x\ge0$ and $x\le1$, and complementarity $x_i z_i=(1-x_i)w_i=0$. The primal--dual interior-point method
...
\end{equation}
The only large linear algebra object in this Newton step is $H_\mu$, a weighted
graph Laplacian with the same sparsity pattern as the network incidence matrix.
Appendix~\ref{app:ipm-elimination} gives the elimination and back-substitution
steps. It remains to specify the complementarity targets $(q_z,q_w)$ and the step
length.

\paragraph{Predictor--corrector direction.} Mehrotra's predictor--corrector method
\citep{mehrotra1992implementation,wright1997primal} uses the current Newton matrix
twice. The first solve asks how much complementarity could be removed by an
aggressive affine step. The second solve recentres that step and corrects the
quadratic error in the products $xz$ and $(1-x)w$. Both solves use the same
factorization of $H_\mu$; only the right-hand side changes.

For any direction, strict positivity is first protected by the
\emph{fraction-to-boundary} lengths
...
\end{aligned}
\end{equation}
where empty minima are ignored. These are the longest primal and dual moves that
keep $x\in(0,1)^n$ and $z,w>0$, backed off by the fixed constant $\tau$.

\emph{Predictor.} The affine predictor sets $q_z=-x\odot z$ and
$q_w=-(\mathbf 1-x)\odot w$, which linearise the complementarity products toward
$0$. Solving gives the affine direction with components
$\Delta x^{\mathrm{aff}}$, $\Delta z^{\mathrm{aff}}$, $\Delta w^{\mathrm{aff}}$
and fraction-to-boundary lengths
$(\alpha^{\mathrm{aff}}_{\mathrm p},\alpha^{\mathrm{aff}}_{\mathrm d})$
from~\eqref{eq:frac-to-boundary}. The affine complementarity measure is the
duality measure of Algorithm~\ref{alg:inner} evaluated at the trial point reached
along this direction,
\begin{equation}\label{eq:mu-aff}
\begin{aligned}
    \mu_{\mathrm{aff}}=\frac{1}{2n}\Bigl[
        (x+\alpha^{\mathrm{aff}}_{\mathrm p}\Delta x^{\mathrm{aff}})^\top
         (z+\alpha^{\mathrm{aff}}_{\mathrm d}\Delta z^{\mathrm{aff}}) + (\mathbf 1-x-\alpha^{\mathrm{aff}}_{\mathrm p}\Delta x^{\mathrm{aff}})^\top
           (w+\alpha^{\mathrm{aff}}_{\mathrm d}\Delta w^{\mathrm{aff}})\Bigr],
\end{aligned}
\end{equation}
i.e., the complementarity that this aggressive step would actually leave. If
$\mu_{\mathrm{aff}}$ is small relative to $\mu$, the affine direction already points
strongly toward the solution and only weak centring is needed. If it is not small,
the method must recenter more.

\emph{Corrector.} The corrector re-solves, with the same $H_\mu$, using the
centring parameter $\sigma=(\mu_{\mathrm{aff}}/\mu)^3$ and the second-order targets
...
Taking the full fraction-to-boundary step along this direction is the usual Mehrotra
heuristic. The next paragraph adds the globalization that turns this heuristic into a
robust algorithm.
...
other products remain large. Such an unbalanced point can be positive but far from
the central path, and bare Mehrotra predictor--corrector methods are not globally
convergent in general \citep{salahi2007mehrotra}. We therefore use the
...
The globalization keeps every trial point in the wide central neighbourhood
\begin{equation}\label{eq:nbhd}
    \cN(\nu)=\bigl\{(x,\lambda,z,w):0<x<\mathbf 1,\ z,w>0,\
        \min_i\bigl(x_iz_i,\ (1-x_i)w_i\bigr)\ge\nu\,\mu\bigr\}.
\end{equation}
Here $\mu=\{x^\top z+(\mathbf 1-x)^\top w\}/(2n)$ is evaluated at the same point, and
$\nu\in(0,1)$ is a fixed centrality constant, unrelated to the sieve coefficients
$\gamma$. Membership in $\cN(\nu)$ means that no lower- or upper-bound
complementarity product is allowed to fall below a fixed fraction of the average.

Given a search direction
$\Delta=(\Delta x,\Delta\lambda,\Delta z,\Delta w)$, scaling its primal and dual
fraction-to-boundary lengths $(\alpha_{\mathrm p},\alpha_{\mathrm d})$
by a common backtracking factor $\alpha\in[0,1]$ gives the trial
point $p(\alpha)$ with components
\begin{equation}\label{eq:trial-point}
    x(\alpha)=x+\alpha\,\alpha_{\mathrm p}\,\Delta x,\qquad
    (\lambda,z,w)(\alpha)=(\lambda,z,w)+\alpha\,\alpha_{\mathrm d}\,(\Delta\lambda,\Delta z,\Delta w),
\end{equation}
whose average complementarity is
\begin{equation}\label{eq:mu-alpha}
    \mu(\alpha)=\tfrac{1}{2n}\bigl[x(\alpha)^\top z(\alpha)+(\mathbf 1-x(\alpha))^\top w(\alpha)\bigr]
\end{equation}
and satisfies $\mu(0)=\mu$.

The algorithm first tries a \emph{fast step}: use the Mehrotra corrector direction and
take the largest backtracked $\alpha$ satisfying
\begin{equation}\label{eq:fast-step}
    p(\alpha)\in\cN(\nu)\quad\text{and}\quad \mu(\alpha)\le\rho\,\mu(0):
\end{equation}
the trial point must remain central and must reduce complementarity by the fixed
factor $\rho<1$. When the full Mehrotra step satisfies these two tests, the
globalization does nothing. When it does not, the algorithm switches to a
\emph{safe step}. The safe step recomputes the Newton direction with a bounded
centring parameter
$\sigma_{\mathrm s}\in[\underline\sigma,\overline\sigma]\subset(0,1)$ and without the
second-order Mehrotra correction, then accepts the largest backtracked $\alpha$ such
that
\begin{equation}\label{eq:safe-step}
    p(\alpha)\in\cN(\nu)
    \quad\text{and}\quad
    \mu(\alpha)\le\bigl(1-\kappa\,\alpha\,(1-\sigma_{\mathrm s})\bigr)\mu(0).
\end{equation}
This is an Armijo decrease condition for the complementarity measure. For the
centred safe direction, the line search accepts a step length bounded away from zero
\citep[Lemma~3.1]{wright1996superlinear}. Hence fallback to a safe step is not a
patch: it is the part of the method that supplies global progress.

The initialization is also chosen to avoid hand tuning. We set
$x=\tfrac12\mathbf 1$ and $\lambda=0$. Then, with
$g=\ell\odot h'(\tfrac12\mathbf 1;\gamma)-v(\beta)$, we choose
$z=\max(g,0)+s\mathbf 1$ and $w=\max(-g,0)+s\mathbf 1$ for a positive common shift
$s$. This makes $z-w=g$, so the stationarity residual at the box centre is zero while
$z,w$ remain strictly positive. The constants in the line searches are fixed across
all instances.
...
\begin{proof}
By the specification of $\Gamma$, $h''(\cdot;\gamma)>0$ on $(0,1)$, so
$F(\cdot;\gamma)=\sum_i\ell_i h(\cdot;\gamma)$ is strictly convex; minimised over the
compact polytope $\cX_b$ this gives the unique $x^*_b(\theta)$ of~\textup{(i)}, and
the KKT map of~\eqref{eq:inner-problem} is monotone. For~\textup{(ii)}, at any
strictly interior iterate the curvature
$\Theta_i=\ell_i h''(x_i;\gamma)+z_i/x_i+w_i/(1-x_i)$ is strictly positive, so
$H_\mu=C\operatorname{diag}(\Theta)^{-1}C^\top$ is positive semidefinite, and positive
definite once the per-component constant nullspace of $C=A$ is grounded; grounding
fixes the multiplier gauge but does not change $x^*_b$. Thus
\eqref{eq:ipm-normal} has a unique grounded solution. This positive definiteness is
the invertibility condition required by the Ralph--Wright theory, and here it follows
from strict convexity. Monotonicity and this invertibility imply existence of a
nonvanishing safe-step length \citep[Lemma~3.1]{wright1996superlinear}, giving
\textup{(iii)}. Slater feasibility keeps the iterates bounded, and the global
convergence theorem for the safe-step/fast-step interior method
\citep[Theorem~1]{ralph2000superlinear} gives~\textup{(iv)}. The superlinear
statement is \citet[Theorem~2]{ralph2000superlinear}; importantly, that result does
not require the active-constraint Jacobian to have full rank, which is useful here
because incidence matrices are rank deficient.
\end{proof}
...
\Statex \textbf{Notation:} $\textsc{Dir}(q_z,q_w)$ solves~\eqref{eq:ipm-normal} and back-substitutes~\eqref{eq:ipm-backsub}; $p(\alpha)$ and $\mu(\alpha)$ are defined in~\eqref{eq:trial-point}--\eqref{eq:mu-alpha}.
...
\For{$k=0,1,2,\ldots$}
    ...
    \EndIf
    \State $\Theta_i\gets\ell_i h''(x_i;\gamma)+z_i/x_i+w_i/(1-x_i)$; factorize $H_\mu\gets C\operatorname{diag}(\Theta)^{-1}C^\top$.
    \State \emph{Predictor:} $\Delta^{\mathrm{aff}}\gets\textsc{Dir}\bigl(-x\odot z,\,-(\mathbf 1-x)\odot w\bigr)$; compute $\mu_{\mathrm{aff}}$ by~\eqref{eq:mu-aff}.
    ...
    \State Compute fraction-to-boundary lengths $(\alpha_{\mathrm p},\alpha_{\mathrm d})$ for $\Delta$ by~\eqref{eq:frac-to-boundary}.
    ...
    \If{$\alpha^\star$ exists}
        \State $(x,\lambda,z,w)\gets p(\alpha^\star)$. \Comment{bare Mehrotra step, taken when admissible}
    \Else
        ...
        \State $(x,\lambda,z,w)\gets p(\alpha)$. \Comment{accepted step length is bounded below}
    \EndIf
\EndFor
\end{algorithmic}
\end{algorithm}
...
\For{$k = 0, 1, 2, \ldots$}
    \For{$b = 1, \ldots, B$ (parallel over OD pairs)}
        \State $(\lambda^*_b(\theta_k),\, x^*_b(\theta_k)) \gets$ Algorithm~\ref{alg:inner} at $\theta_k$.
    \EndFor
    \State Assemble $g_k = \nabla Q_B(\theta_k)$ from~\eqref{eq:outer-gradient} using $\{x^*_b(\theta_k)\}_b$.
...
back-substitution formulas~\eqref{eq:ipm-backsub}. Start from the Newton system
\eqref{eq:newton-system}:
\[
\begin{aligned}
    \operatorname{diag}(\ell\odot h''(x;\gamma))\Delta x
        +C^\top\Delta\lambda-\Delta z+\Delta w &= -r_{\mathrm d},\\
