...
\paragraph{Newton solve.} At a strictly interior iterate $(x,\lambda,z,w)$, define
\[
    r_{\mathrm d}=\ell\odot h'(x;\gamma)-v(\beta)+C^\top\lambda-z+w,
    \qquad
    r_{\mathrm p}=Cx-d.
\]
Let $(q_z,q_w)$ be the desired linearized changes in the two complementarity
products. Linearization gives
...
\end{aligned}
\end{equation}
Eliminating $\Delta z$, $\Delta w$, and $\Delta x$ reduces this system to one
equation for $\Delta\lambda$:
...
\]
The remaining directions are then recovered by back-substitution:
...
The remaining choices are the targets $(q_z,q_w)$ and the step length.

\paragraph{Predictor--corrector direction.} Mehrotra's method
\citep{mehrotra1992implementation,wright1997primal} chooses $(q_z,q_w)$ with two
right-hand sides and one factorization of $H_\mu$. The affine predictor sets
$q_z=-x\odot z$ and $q_w=-(\mathbf 1-x)\odot w$, asking how much complementarity
would remain if the linearized products were driven toward zero.
...
Let $(\Delta x^{\mathrm{aff}},\Delta\lambda^{\mathrm{aff}},
\Delta z^{\mathrm{aff}},\Delta w^{\mathrm{aff}})$ be the affine direction, and let
$(\alpha^{\mathrm{aff}}_{\mathrm p},\alpha^{\mathrm{aff}}_{\mathrm d})$ be its
fraction-to-boundary lengths. The affine complementarity is
...
\end{aligned}
\end{equation}
the average complementarity after the largest admissible affine move.

The corrector uses
$\sigma=(\mu_{\mathrm{aff}}/\mu)^3$. When the affine step is effective, $\sigma$ is
small and the method follows it aggressively; otherwise $\sigma$ increases
centring. The corrected targets are
...
Taking the full fraction-to-boundary step along $\Delta$ is the bare Mehrotra
heuristic. The safeguard below decides whether that step is acceptable.
...
own average. The multiplier $\lambda$ is included in $y$ because it is part of the
iterate, but centrality depends only on $(x,z,w)$.

Now fix the current iterate
$y_k=(x_k,\lambda_k,z_k,w_k)$ and write $\mu_k=\bar\mu(y_k)$. Given a search
direction $\Delta$, scale its primal and dual fraction-to-boundary lengths by a
common backtracking factor $\alpha\in[0,1]$:
...
The line search first tries a \emph{fast step}: use the Mehrotra corrector direction
and take the largest backtracked $\alpha$ satisfying
\begin{equation}\label{eq:fast-step}
    y_k(\alpha)\in\cN(\nu)\quad\text{and}\quad \mu_k(\alpha)\le\rho\,\mu_k.
\end{equation}
If the full Mehrotra step satisfies~\eqref{eq:fast-step}, the safeguard is
transparent. If no fast step is accepted, we recompute a centred safe direction with
$\sigma_{\mathrm s}\in[\underline\sigma,\overline\sigma]\subset(0,1)$ and no
second-order correction. The accepted safe step is the largest backtracked $\alpha$
satisfying
...
\end{equation}
For the centred direction, this Armijo line search accepts a step length bounded
away from zero \citep[Lemma~3.1]{wright1996superlinear}; this is the global progress
mechanism.
