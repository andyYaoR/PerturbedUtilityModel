...
active-set structure at the optimum. For a multiplier $\lambda$, define
...
\paragraph{Why a pure active-set Newton method is not enough.} The dual objective
is convex and smooth as long as no recovered coordinate is at a bound. At a
saturated coordinate, however, the derivative of $\xi^*$ is zero and the
corresponding curvature weight disappears. A semismooth Newton method therefore
uses a matrix of the form $C\operatorname{diag}(D)C^\top$, with
$D_i=1/(\ell_i h''(x_i;\gamma))$ on links satisfying $0<x_i<1$ and $D_i=0$ at
the bounds. This is fast once the correct active set is known. On large cold
starts, especially for sparse polynomial PURC solutions, the active set can be
wrong for many iterations. We therefore use an interior method to reach a
well-centered point near the solution before switching to the exact active-set
solver.

\paragraph{Solver regime.} For the polynomial sieve used in this paper,
$h'$ is finite on the closed interval $[0,1]$. The inner solver is therefore a
primal--dual interior-point method for the box constraints, followed by a
semismooth Newton crossover that removes the remaining barrier bias. This is the
default route for smooth-on-the-box perturbations such as the quadratic and the
polynomial sieve. For perturbations whose derivative diverges at a box face
(for example entropy kernels), a primal barrier at that face is ill posed; those
cases are solved directly in the dual, with a barrier-smoothed dual continuation
available as a robust fallback. The paper's estimator only needs the polynomial
sieve case, but the implementation uses this regime rule so that the same
software handles several perturbation families without hand tuning.
...
\end{equation}
For fixed $\mu$, the iterate stays strictly inside the box:
$0<x_i<1$ and $z_i,w_i>0$. Equivalently, the primal component solves the
log-barrier problem
\[
    \min_{Cx=d}\;
    F(x;\gamma)-v(\beta)^\top x
    -\mu\sum_i\{\log x_i+\log(1-x_i)\}.
\]
...
$H_\mu$ uses the full network graph. Near the solution, links that saturate have
vanishing weights, while links with $0<x_i^*<1$ retain the curvature weights
$1/(\ell_i h''(x_i^*;\gamma))$. Under Assumption~\ref{ass:neg_utility}, the
positive-flow active subgraph is acyclic.
\end{lemma}
...
\paragraph{Crossover to the exact box solution.} The interior-point iterate is
strictly interior, whereas the PURC solution is often sparse. We therefore use
the IPM only to obtain a reliable multiplier near the solution. Once primal and
dual residuals and complementarity are small, we pass $\lambda$ to the exact
semismooth Newton solver for the original box problem. This crossover removes
the barrier approximation and identifies the final active set
$S_b^*(\theta)=\{i:0<x^*_{i,b}(\theta)<1\}$. In practice this also recovers the
sparser active-subgraph Laplacian near the optimum.
...
\For{$k = 0, 1, 2, \ldots$}
    ...
    \If{$\|(r_{\mathrm d},r_{\mathrm p})\|_\infty$ and $\mu$ are below the crossover tolerances} \textbf{break} \EndIf
...
\EndFor
\State Run semismooth Newton on the original box problem, warm-started at $\lambda$.
\State \textbf{return} $(\lambda_b^*(\theta),\,x_b^*(\theta),\,S_b^*(\theta))$.
\end{algorithmic}
\end{algorithm}
...
\For{$k = 0, 1, 2, \ldots$}
    \For{$b = 1, \ldots, B$ (parallel over OD pairs)}
        \State $(\lambda^*_b(\theta_k),\, \hat{x}_b(\theta_k),\, \mathcal{S}_b) \gets$ Algorithm~\ref{alg:inner} at $\theta_k$.
    \EndFor
