...
\paragraph{Globalization and convergence.} Fraction-to-boundary preserves
positivity but not centrality. To globalize the Mehrotra step, we use the
safe-step/fast-step line search of
\citet{wright1996superlinear,ralph2000superlinear}, within the framework of
\citet{kojima1994global}. For $y=(x,\lambda,z,w)$, define
...
\end{equation}
Thus $\cN(\nu)$ excludes strictly positive but badly unbalanced iterates; the
average in~\eqref{eq:nbhd} is evaluated at the same point $y$.

At iteration $k$, let $y_k=(x_k,\lambda_k,z_k,w_k)$ and
$\mu_k=\bar\mu(y_k)$. For a direction $\Delta$, let
$(\alpha_{\mathrm p},\alpha_{\mathrm d})$ be the fraction-to-boundary primal and
dual lengths. The trial point and its complementarity are
...
\end{equation}
The fast step takes the largest backtracked $\alpha$ along the Mehrotra corrector
direction satisfying
\begin{equation}\label{eq:fast-step}
    y_k(\alpha)\in\cN(\nu)\quad\text{and}\quad \mu_k(\alpha)\le\rho\,\mu_k.
\end{equation}
If this fails, the safe step recomputes a centred direction with
$\sigma_{\mathrm s}\in[\underline\sigma,\overline\sigma]\subset(0,1)$, omits the
second-order correction, and accepts the largest backtracked $\alpha$ satisfying
\begin{equation}\label{eq:safe-step}
    y_k(\alpha)\in\cN(\nu)
    \quad\text{and}\quad
    \mu_k(\alpha)\le\bigl(1-\kappa\,\alpha\,(1-\sigma_{\mathrm s})\bigr)\mu_k.
\end{equation}
The safe-step line search accepts a step length bounded away from zero
\citep[Lemmas~3.1--3.2]{wright1996superlinear}; this is the global progress
guarantee.
...
$\cX_b$ contains a strictly interior flow ($0<x<\mathbf 1$, $Cx=d$). Then the
safeguarded Algorithm~\ref{alg:inner} is globally convergent: every limit point is a
primal--dual optimum of~\eqref{eq:inner-problem}, and its primal component is the
unique minimiser $x^*_b(\theta)$. If the solution is strictly complementary, the
local rate is superlinear.
\end{proposition}

\begin{proof}
Since $\gamma\in\Gamma$, $h''(\cdot;\gamma)>0$ on $(0,1)$. Thus
$F(\cdot;\gamma)$ is strictly convex, so the inner problem has a unique primal
minimiser and its KKT map is monotone. At any strictly interior iterate
...
space. Together with the assumed strict feasibility, these are the monotonicity,
Slater, and nonsingular reduced-Newton conditions required by the
safe-step/fast-step interior-point theory. The safe-step length bound is
\citet[Lemmas~3.1--3.2]{wright1996superlinear}. Global convergence follows from
\citet[Theorem~3.3]{wright1996superlinear} and its constrained variational-inequality
counterpart \citet[Theorem~1]{ralph2000superlinear}. Under strict complementarity,
local superlinear convergence follows from \citet[Theorem~2]{ralph2000superlinear},
which does not require the active-constraint Jacobian to have full rank.
\end{proof}
...
\For{$k=0,1,2,\ldots$}
    ...
    \State Compute fraction-to-boundary lengths for $\Delta$.
    \State \emph{Fast step:} let $\alpha^\star$ be the first $\alpha\in\{1,\chi,\chi^2,\ldots\}$ satisfying~\eqref{eq:fast-step}, if any.
    \If{$\alpha^\star$ exists}
        \State $(x,\lambda,z,w)\gets y_k(\alpha^\star)$.
    \Else
        \State $\sigma_{\mathrm s}\gets\min(\overline\sigma,\max(\underline\sigma,\sigma))$.
        \State $\Delta\gets\textsc{Dir}\bigl(\sigma_{\mathrm s}\mu\mathbf 1-x\odot z,\ \sigma_{\mathrm s}\mu\mathbf 1-(\mathbf 1-x)\odot w\bigr)$.
        \State Recompute fraction-to-boundary lengths and backtrack until~\eqref{eq:safe-step} holds.
        \State $(x,\lambda,z,w)\gets y_k(\alpha)$.
    \EndIf
\EndFor
\end{algorithmic}
\end{algorithm}
