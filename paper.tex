...
The globalization keeps every trial point in the wide central neighbourhood. To
avoid overloading notation, define the average complementarity of any strictly
interior point $y=(x,\lambda,z,w)$ by
\begin{equation}\label{eq:mu-bar}
    \bar\mu(y)
    :=\bar\mu(x,z,w)
    =\frac{x^\top z+(\mathbf 1-x)^\top w}{2n}.
\end{equation}
The neighbourhood is then the set of points whose \emph{own} complementarity
products are not too uneven:
\begin{equation}\label{eq:nbhd}
    \cN(\nu)=\bigl\{y=(x,\lambda,z,w):0<x<\mathbf 1,\ z,w>0,\
        \min_i\bigl(x_iz_i,\ (1-x_i)w_i\bigr)\ge\nu\,\bar\mu(y)\bigr\}.
\end{equation}
The multiplier $\lambda$ appears in $y$ because it is part of the primal--dual
iterate, but centrality itself depends only on $(x,z,w)$. The constant
$\nu\in(0,1)$ is fixed and is unrelated to the sieve coefficients $\gamma$.
Membership in $\cN(\nu)$ means that no lower- or upper-bound complementarity
product is allowed to fall below a fixed fraction of that point's average.

Now fix the current iterate
$y_k=(x_k,\lambda_k,z_k,w_k)$ and write $\mu_k=\bar\mu(y_k)$. Given a search
direction $\Delta=(\Delta x,\Delta\lambda,\Delta z,\Delta w)$, scaling its primal
and dual fraction-to-boundary lengths $(\alpha_{\mathrm p},\alpha_{\mathrm d})$ by a
common backtracking factor $\alpha\in[0,1]$ gives the trial point $y_k(\alpha)$ with
components
...
\end{equation}
At $\alpha=0$, the trial point is exactly the current iterate, so
$y_k(0)=y_k$ and $\mu_k(0)=\bar\mu(y_k)=\mu_k$.
...
\Statex \textbf{Notation:} $\textsc{Dir}(q_z,q_w)$ solves~\eqref{eq:ipm-normal} and back-substitutes~\eqref{eq:ipm-backsub}; $y_k(\alpha)$ and $\mu_k(\alpha)$ are defined in~\eqref{eq:trial-point}--\eqref{eq:mu-alpha}.
